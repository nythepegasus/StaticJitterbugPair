...
int main(int argc, const char * argv[]) {
    ...
    while ((c = getopt(argc, (char *const *)argv, "lu:c")) != -1) {
        switch (c) {
            case 'l': {
                return print_udids();
            }
    ...
    if (lerr != LOCKDOWN_E_SUCCESS) {
        fprintf(stderr, "ERROR: lockdown_pair returned '%d'\n", lerr);
        result = EXIT_FAILURE;
    ...
    plist_err_t plist_write_err = plist_write_to_file(pair_record, path, 1, PLIST_FORMAT_XML);
    if (plist_write_err != 0) {
        fprintf(stderr, "ERROR: plist_write_to_file returned '%d'\n", plist_write_err);
        result = EXIT_FAILURE;
        goto leave;
    }
